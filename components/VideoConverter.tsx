...
import {
  ...
  VideoFormat,
  videoFormatCategories,
} from "@/lib/formats/videoFormats";
...
type VideoStatus = "idle" | "converting" | "success" | "failed";

type VideoOptions = {
  resolution: "auto" | "480p" | "720p" | "1080p" | "1440p" | "4k";
  quality: "auto" | "low" | "medium" | "high";
  videoCodec: "auto" | "h264" | "h265" | "vp9" | "av1";
  audioCodec: "auto" | "aac" | "mp3" | "opus" | "vorbis";
  removeAudio: boolean;
  trimStart: string;
  trimEnd: string;
};
...
function isSupportedVideo(file: File) {
  return (
    ...
    /\.(3g2|3gp|3gpp|avi|cavs|dv|dvr|flv|m2ts|m4v|mkv|mod|mov|mp4|mpeg|mpg|mts|mxf|ogg|ogv|rm|rmvb|swf|ts|vob|webm|wmv|wtv)$/i.test(
      file.name,
    )
  );
}
...
export function VideoConverter() {
  ...
  const [draftOptions, setDraftOptions] = useState<VideoOptions>(defaultVideoOptions);
  ...
  async function selectFile(file: File | undefined) {
    ...
    if (!isSupportedVideo(file)) {
      setStatus("failed");
      setMessage("Unsupported file. Choose a supported video format.");
      return;
    }

    if (file.size > 250 * 1024 * 1024) {
      setStatus("failed");
      setMessage("File too large. Try a video under 250 MB.");
      return;
    }
    ...
    setSelectedVideo({
      ...
      durationLabel: await readVideoDuration(file),
    });
  }
  ...
  async function convertVideo() {
    if (!selectedVideo) {
      setStatus("failed");
      setMessage("Upload one video to start.");
      return;
    }
    ...
    formData.append("file", selectedVideo.file, selectedVideo.file.name);
    ...
    if (!response.ok) {
      setStatus("failed");
      setMessage(payload?.error ?? "Video conversion failed. Please try again.");
      return;
    }
  ...
  const convertDisabled =
    status === "converting" || !selectedVideo || !usage || usage.remaining <= 0;

  return (
    <>
      <Navbar user={user} usage={usage} onOpenAuth={openAuth} onLogout={handleLogout} />
      <main className="converter-page" data-tool-kind="video">
            ...
            <div className="converter-shell card-shadow rounded-[1.5rem] border bg-[var(--card)] p-4 sm:p-6 lg:p-7">
              <div>
                ...
                <h1 className="mt-4 text-2xl font-semibold text-[var(--foreground)] sm:text-3xl">
                  Convert videos without the hassle
                </h1>
                <p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">
                  Upload a video, choose your output format, and convert it in a few clicks.
                </p>
              </div>
              ...
              <div className="mt-7 space-y-4">
                ...
                <div className="rounded-[1.5rem] border bg-[var(--card-muted)] p-3 sm:p-3.5">
                  <div className="grid gap-3 md:grid-cols-2 md:items-start">
                    <VideoFormatDropdown value={outputFormat} onChange={setOutputFormat} />
                ...
                {status === "success" && selectedVideo ? (
                  <div className="rounded-[1.5rem] border bg-[var(--card-muted)] p-5">
                    ...
                    <p className="mt-2 text-sm text-[var(--muted-foreground)]">
                      {selectedVideo.file.name} - {outputFormat}
                    </p>
                    <button
                      type="button"
                      onClick={() => {
                ...
                <div className="flex gap-3 rounded-[1.25rem] border bg-[var(--card-muted)] px-4 py-3 text-sm text-[var(--muted-foreground)]">
                  ...
                  <p className="leading-6">
                    Video files are checked before conversion. Guest limits reset daily.
                  </p>
                </div>
              </div>
            </div>
      ...
      {isOptionsOpen ? (
        <VideoOptionsModal
          ...
          onDone={applyOptions}
          onReset={() => setDraftOptions(defaultVideoOptions)}
        />
      ) : null}
    </>
  );
...
function VideoFormatDropdown({
  ...
  const [activeCategory, setActiveCategory] = useState(videoFormatCategories[0].label);
  const rootRef = useRef<HTMLDivElement | null>(null);

  const activeFormats = useMemo(() => {
    const category =
      videoFormatCategories.find((item) => item.label === activeCategory) ??
      videoFormatCategories[0];
...
function VideoOptionsModal({
  ...
  draftOptions: VideoOptions;
  setDraftOptions: (options: VideoOptions) => void;
      ...
      >
        <div className="border-b px-4 py-3">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-sm font-semibold text-[var(--foreground)]">Video options</p>
              <p className="mt-0.5 text-xs text-[var(--muted-foreground)]">
                Adjust quality, resolution, and audio settings before converting.
              </p>
            </div>
            <button
              type="button"
        ...
        <div className="grid gap-3 overflow-y-auto p-4 sm:grid-cols-2">
          <OptionSelect
            ...
            label="Resolution"
            value={draftOptions.resolution}
            options={["auto", "480p", "720p", "1080p", "1440p", "4k"]}
            labels={{ auto: "Auto", "4k": "4K" }}
            onChange={(value) => updateOption("resolution", value as VideoOptions["resolution"])}
          />
          <OptionSelect
            label="Quality"
            ...
            onChange={(value) => updateOption("quality", value as VideoOptions["quality"])}
          />
          <OptionSelect
            label="Video codec"
            value={draftOptions.videoCodec}
            options={["auto", "h264", "h265", "vp9", "av1"]}
            labels={{ auto: "Auto", h264: "H.264", h265: "H.265", vp9: "VP9", av1: "AV1" }}
            onChange={(value) => updateOption("videoCodec", value as VideoOptions["videoCodec"])}
          />
          <OptionSelect
            label="Audio codec"
            value={draftOptions.audioCodec}
            options={["auto", "aac", "mp3", "opus", "vorbis"]}
            labels={{ auto: "Auto", aac: "AAC", mp3: "MP3", opus: "Opus", vorbis: "Vorbis" }}
            onChange={(value) => updateOption("audioCodec", value as VideoOptions["audioCodec"])}
          />
          <div className="space-y-1">
            <span className="text-xs font-medium text-[var(--muted-foreground)]">Remove audio</span>
            <div className="flex min-h-10 items-center gap-4 rounded-xl border bg-[var(--card-muted)] px-3 text-sm text-[var(--foreground)]">
              <label className="inline-flex items-center gap-2">
                <input
                  type="radio"
                  name="remove-audio"
                  checked={draftOptions.removeAudio}
                  onChange={() => updateOption("removeAudio", true)}
                />
                Yes
              </label>
              <label className="inline-flex items-center gap-2">
                <input
                  ...
                  name="remove-audio"
                  checked={!draftOptions.removeAudio}
                  onChange={() => updateOption("removeAudio", false)}
                />
                No
              </label>
            </div>
